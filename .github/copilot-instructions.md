...
- Keep trailing slashes on section URLs (`/projects/` not `/projects`) to match current routing and `main.js` nav matching.
- Reuse the existing page shell on all pages:
  ...
  3) footer with a real fallback year inside `<span data-year>`
  4) `<script src="/assets/js/main.js"></script>`
- Home-only presentation depends on `body.home-page`; do not add this class to non-home pages.
...
- Curated project evidence belongs under `assets/projects/<project-slug>/`; keep private or unverified material out of the deployed site.
...
- When changing shared behavior, update both CSS and JS consistently and preserve the no-JavaScript content fallback.
